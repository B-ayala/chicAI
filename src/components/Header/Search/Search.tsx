...
export default function Search({ancho,margen,formType,closeMenu}:SearchProps){
    ...
    const datos: any = [
        "ZAPATOS", "PANTALONES", "REMERAS", "CALZADO", "CAMIZAS"
    ]
    ...
    const handleChangeInput = (change: string) => {
        const filterProducts = featuredProducts.filter((item) => (item.name.toLocaleUpperCase()).includes(change.toLocaleUpperCase())&&change)
    ...
    const handleClick = (value:number,name:string)=>{
        const productsFilter = featuredProducts.filter((product)=>product.id == value)
    ...
    const handleClickSearch = ()=>{
        const filterProducts = featuredProducts.filter((item) => (item.name.toLocaleUpperCase()).includes(input.toLocaleUpperCase()))
    ...
    return (
        <>
            <div style={{ marginLeft: margen, zIndex:2 }}>
                ...
                {
                    datos.length > 0 &&
                    <Box width={ancho} sx={{ background: "#ebe1e0ff", fontSize: "20", listStyleType: "none", position: "absolute" }}>
                        {
                            products.map((item) => (
                                
                                <TextSearch   key={item.id} as={Link} to={"/products"}  onClick={()=>handleClick(item.id,item.name)}>
                                    {item.name}
